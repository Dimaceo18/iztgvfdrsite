...
YOUR_TELEGRAM_ID = os.getenv('YOUR_TELEGRAM_ID')  # Твой личный ID для получения кнопок
    ...
    try:
        ...
        if channel_post.photo:
            try:
                ...
                media_id = await download_and_upload_photo(photo.file_id)
            except Exception as e:
                logger.error(f"Ошибка фото: {e}")
        ...
        logger.info(f"✉️ Отправлен запрос на публикацию поста '{title[:50]}...'")
...
async def handle_button_click(update: Update, context):
    ...
    await query.edit_message_text(
        f"⏳ <b>Обработка...</b>\n\n"
        f"Пост '{post_data['title'][:50]}...' {status_text}.",
        parse_mode='HTML'
    )
    ...
    if success:
        ...
        await query.edit_message_text(
            f"❌ <b>Ошибка!</b>\n\n"
            f"Не удалось {status_text} пост '{post_data['title'][:100]}'.\n\n"
            f"Проверь логи для деталей.",
            parse_mode='HTML'
        )
        logger.error(f"❌ Ошибка при создании поста: {post_data['title'][:50]}")
...
if __name__ == '__main__':
    ...
    if not YOUR_TELEGRAM_ID:
        logger.error("❌ YOUR_TELEGRAM_ID не задан!")
        logger.info("💡 Как узнать свой ID: напиши @userinfobot в Telegram")
